...
PhoneGap.onDeviceReady.subscribeOnce(function() {
	//ianic iskljucio ovo, vartio se na klasican sistem
  //PhoneGap.JSCallback();
});
...
PhoneGap.execAsync = function(success, fail, clazz, action, args) {
    try {
				console.log("calling PhoneGap.execAsync action:" + action);
    ...
    xmlhttp.onreadystatechange=function(){
        if(xmlhttp.readyState == 4){
            ...
            if (xmlhttp.status == 200) {
							  ...
							  if (msg.startsWith('<?xml version="1.0"?>')){ 
									//console.log("JSCallback invalid response, forcing restart callback server");
									console.log(" status:" + xmlhttp.status + " statusText: " + xmlhttp.statusText);
									console.log("JSCallback invalid response: " + msg);
									//CallbackServer.restartServer();
									setTimeout(PhoneGap.JSCallback, 1000);									
									return;
							  }
								console.log("JSCallback xmlhttp.responseText: " + msg);								
            ...
            // If error, restart callback server
                ...
                console.log("JSCallback Error: Request failed. status:" + xmlhttp.status + " statusText: " + xmlhttp.statusText );
                CallbackServer.restartServer();
  	...
  	console.log("JSCallback start listening on: " + "http://127.0.0.1:"+CallbackServer.getPort()+"/");
